...
CT's approach is architectural rather than behavioral. It does not ask models to be less sycophantic or more accurate. It changes the interaction geometry so that a different set of pressures operate — multiple simultaneous perspectives, no single social pressure point, model-to-model evaluation that removes the user's ego investment from the evaluative loop. The geometry does not remove social pressure from the structure; it changes which pressure operates and where it can be disrupted. Sequential exposure in Conductor mode, for instance, can produce anchoring — earlier responses shaping later ones in ways that resemble triangulation but function as deference — which is a different failure mode than user-directed sycophancy, addressed by different structural levers (routing, exclusion, ordering) rather than escaped by the architecture altogether.
