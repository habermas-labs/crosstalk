...
| 057 | `[CT]` | API key pre-flight validation | "Test" button adjacent to each API key field in Settings; fires a minimal test call to each provider and returns a green/red status with a human-readable diagnosis line displayed beneath the key field. Red states must name the specific failure: invalid key (401/403), Gemini quota:0 (free-tier provisioning failure — distinct from exhausted quota, fix is to create a new key in a new Google project), or network/service unreachable. Intended as a gate before `.ctk` export: user confirms all three keys are live before packaging. Interim manual procedure documented in CT-058 until this is implemented. | `planned` | Session discussion | 2026-03 |
