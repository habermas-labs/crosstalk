...
| 045 | Focus mode | Single-model sub-session within CT; one model active, others disabled; useful for drafting, deep dives, cost control; CT epistemic infrastructure still present | `idea` | Session discussion | 2026-03-03 |
| 046 | RIFF mode | Structured role-based triangulation: three models in assigned epistemic roles (Presenter, Critical Reader, Silent Observer) with the human as Conductor; named after Bakhtin's dialogism; primary triangulation mode | `idea` | Session discussion | 2026-03-03 |
...
| B025 | Model switching unresponsive when default model unavailable | setPendingModels nested inside setSelectedModels updater caused React batching to diverge pending/selected state; 'default' sentinel could propagate into selectedModels via confirm flow; fix: decoupled state updates, fetchSucceeded guard on both | `resolved` | 2026-06-17 | 2026-06-17 |
| B026 | Model confirm button easy to miss | Bottom-of-list "Confirm changes" button appeared only after making a change and was easy to miss; fix: replaced with always-visible ✕/✓ header buttons in Models section label | `resolved` | 2026-06-17 | 2026-06-17 |
